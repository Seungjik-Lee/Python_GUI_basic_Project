...
#파일 추가
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", ".png"), ("모든 파일", "*.*")), initialdir=r"C:\Users\user\Desktop\파이썬 공부\python GUI_basic_project") #최초에 c:/경로를 보여줌
...
def merge_image():
    ...
    try:
        ...
        for idx, img in enumerate(images):
            ...
            progress = (idx + 1) / len(images) * 100 # 실제 persent 정보를 계산
            p_var.set(progress)
            progress_bar.update()

        #포맷 옵션 처리
        file_name = "SJ_photo." + img_format
